...
**Phase 5 — personalization, skill, and distribution.** Convention extraction, style memory, failure knowledge, the skill doc itself. Token measurement framework. Cloud provider implementations (Anthropic, OpenAI, Qwen-cloud) drop into the `mycel-models` crate as additional `Embedder`/`Synthesizer`/`Reranker` impls — no architecture change needed. Ongoing iteration on the skill.

Distribution workstream (Phase 5): the v1 shipping shape is **`mycel daemon` manages its own `redis-server` + `falkordb.so` subprocess, bound to a per-repo Unix socket at `~/.local/share/mycel/<repo>/falkordb.sock`** — not a TCP port. The Docker Compose path in `docker/docker-compose.yml` stays as the "advanced isolation" fallback, but it is *not* the paved road. Rationale: every dev machine that already runs Redis (a non-trivial fraction) hits a port-6379 conflict the moment they try `just up`; a Unix-socket subprocess sidesteps the entire class. Mycelium's storage layer should be invisible to the user by default — they should not need to know Redis exists, let alone manage it. Brew-tap install (`brew install gavdevs/mycel/mycel`) pulls the platform-matched `redis-server` + `falkordb.so` from the upstream FalkorDB tap. Power users who want to point Mycelium at an existing FalkorDB-loaded Redis can set `storage.falkordb_url` in `~/.config/mycel/config.toml` (the config slot already exists at `crates/mycel-cli/src/config.rs`); at that point they take responsibility for module-loaded compatibility, version pinning, namespace collisions on graph names, and lifecycle entanglement.
...
- **BYO-Redis vs. managed-subprocess FalkorDB.** Default is managed-subprocess on a per-repo Unix socket (see Phase 5 distribution note). Users who already have a FalkorDB-loaded Redis they want Mycelium to share can point at it via `storage.falkordb_url` in `~/.config/mycel/config.toml` or `MYCEL_FALKORDB_URL`. Reuse is opt-in, never the default, because: graphs are namespaced as `mycel:<repo-name>` and would land in the user's instance; FalkorDB version compatibility is not negotiated at runtime (Mycelium pins `v4.18.3`); the user's Redis lifecycle is outside Mycelium's control. Windows users may need to fall back to the Docker Compose path until a Windows-native `redis-server` + `falkordb.so` ships — the FalkorDB tap's Windows binaries are weak as of 2026-05.
